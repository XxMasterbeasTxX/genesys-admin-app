...
- **Flows — Flow Overview (new, internal)** — interactive graphical overview of an Architect callflow and all of its dependency flows, built from the flow's **structured Archy/SDK YAML** (not the flat REST `latestconfiguration`, which omits the implicit *Default* reconvergence links). Pick a live flow and every task, action, decision/switch/menu, and **loop** is drawn as an ELK-laid-out SVG — loops render as **nested container boxes** (matching Architect) and each task starts at its true entry node. A tab per dependency flow (common modules, in-queue, transfer/bot targets — any type, incl. outbound) is discovered by matching referenced flow names to the org's flow list. High/mid/low detail levels, dark/light/white themes, fullscreen, a **variable & dependency cross-reference search** (click a variable to jump to where it's set/used, or a dependency to open its flow), node-level details (values set in that node, conditions/expressions, referenced flow/task, variables used), and **PDF / HTML / JSON download** — either the current flow or a **Save all** that auto-loads the full transitive dependency closure and bundles every flow into one file (multi-page PDF, tabbed HTML, or combined JSON). Read-only. The SPA calls the internal `POST /api/flow-yaml`, which forwards to the onboarding runner's `POST /api/export-yaml` (SDK export, shared-secret `x-export-key`). New SWA app settings: `RUNNER_BASE_URL` + `EXPORT_YAML_KEY`. Access key: `flows.flowoverview`.
...
- **Flows — Flow Overview** *(internal only)* — Interactive, read-only graphical overview of an Architect callflow and its dependency flows. Pick an org and a live flow from a searchable combobox; the flow's **structured Archy/SDK YAML** is exported on demand and parsed into a node/edge model (the flat REST config is not used because it omits implicit *Default* reconvergence edges). Rendered as an ELK-laid-out SVG where each **task** and each **loop** is a container box (loops nest inside their task, inner loops inside outer loops) and every task starts at its true entry action. Decisions, switches (incl. the default case), menus, data-table lookups (Found/Not Found), data actions (Success/Failure), common-module calls, transfers, and jumps all get labelled edges. A **tab per dependency flow** — common modules, in-queue flows, transfer/bot targets of any type including outbound — is discovered by matching referenced flow names to the org's flow list. Detail levels (High: full task/action graph, Mid: task + dependency graph, Low: flow + dependencies). Dark/light/white themes, OS + in-app fullscreen. Clicking a node shows the values **set in that node**, its condition/expression, referenced flow or called task, and variables used; clicking an edge shows From/To and centres either end. A combined **variable & dependency search** lists everywhere a variable is set/used (click a result to jump to that node) and every dependency (click to open its flow). Download as **PDF, HTML (self-contained), or JSON** — either the current flow or **Save all**, which auto-loads the full transitive dependency closure and bundles every flow into one file (multi-page PDF, tabbed HTML, or combined JSON). Backed by the internal `POST /api/flow-yaml` → onboarding runner `POST /api/export-yaml`. Access key: `flows.flowoverview`.
...
│   │   ├── flowYaml.js           Flow Overview: Archy YAML → node/edge model (loops as nested containers)
...
│   │   ├── flows/
│   │   │   ├── flowOverview.js      Flow Overview — interactive SVG overview of a flow + dependencies (ELK layout, nested loops, Save-all export)
