...
- **Step 5 (in progress): customer login path** — `GET /api/org-config?org=<slug>` now also serves an **unauthenticated pre-login** lookup that returns a customer org's **public** OAuth login config (`{ id, name, region, clientId }`) so the SPA can build the authorize URL against the customer's own org before login. Only public fields are returned — never secrets, entitlements, or other orgs. Client helper: `js/services/orgConfigService.js::fetchOrgLoginConfig`. Dynamic login wiring (customer `clientId` + region) and entitlement-driven access follow in later sub-steps; internal login is unchanged.
