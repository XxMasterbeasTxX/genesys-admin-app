...
- **WebRTC Phones — Delete** *(Master Admin only)* — Find WebRTC phones nobody is using — no user assigned, or a holder who is deleted or inactive — review them by category, then delete the confirmed selection; phones that could not be read are reported and locked rather than assumed unused, and each phone is re-checked immediately before removal
