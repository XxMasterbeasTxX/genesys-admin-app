...
- **Interaction Totals** — Visualise interaction counts by Media Type, Direction, and Routing Method as horizontal bar charts. Date range picker with quick-select presets: Last Week (ISO Mon–Sun), Last Month, Last 3 Months, Last Year. Optional Media Type and Direction filters narrow the API query. Uses the async analytics jobs API (`POST /api/v2/analytics/conversations/details/jobs`) with `pageSize=10000` cursor pagination to fetch all conversations, then tallies counts client-side. **Export to Excel** produces a styled summary workbook with title rows (Interaction Totals, Org name, Period, Filters) above a Category/Value/Count/Percentage data table. **Email** section with toggle, recipients, and message to send the Excel as an attachment via Mailjet. **Schedule** panel for automated daily/weekly/monthly export with period preset dropdown (Last Week / Last Month / Last 3 Months / Last Year). Server-side handler in `api/lib/exports/interactionTotals.js`. Access key: `export.interactions.totals`.
