...
- **Recordings — Create Export Job** — Create a Genesys Cloud bulk recording export job via the native `POST /api/v2/recording/jobs` API. Filters: date range (required), Integration dropdown (auto-fetched from the org via `GET /api/v2/integrations`, filtered to supported storage types `amazon-s3`, `azure-blob-storage`, `google-cloud-storage` — a yellow warning is shown if none are configured), Queues multi-select, Users multi-select, Include Screen Recordings toggle, optional Action Date. The job is created in the `READY` state — recordings remain in Genesys until the job is executed from the Export Jobs page. On submit the new Job ID is shown with a link to the Export Jobs page. Access key: `interactions.recordings.create`.
- **Recordings — Export Jobs** — List, manage, and inspect all recording jobs (`EXPORT`, `DELETE`, `ARCHIVE`) in the selected org. Table columns: Created, State (colour-coded badge: PENDING/READY/PROCESSING/FULFILLED/CANCELLED/FAILED), Conversations, Estimated Recordings, Created By, Actions. Per-row expand (▶/▼) reveals the original filters used to create the job: Date Range, Integration name (resolved from `/api/v2/integrations/{id}`), Queues (resolved to names), Users (resolved to names), Screen Recordings, Action Date, Created By. Name resolution is cached in memory to avoid repeated API calls. Actions: **Execute** (only on `READY` jobs — `PUT /api/v2/recording/jobs/{id}` with `{state:"PROCESSING"}` — triggers the actual upload to the configured storage integration), **Cancel** (PUT with `{state:"CANCELLED"}`), **Delete** (`DELETE /api/v2/recording/jobs/{id}`). Refresh button reloads all job types in parallel and preserves the expanded row. Access key: `interactions.recordings.jobs`.
