...
- **Deployment — Flow Overview (new, internal)** — interactive graphical overview of an Architect callflow and all of its dependency flows, built from the flow's **structured Archy/SDK YAML** (not the flat REST `latestconfiguration`, which omits the implicit *Default* reconvergence links). Pick a live flow and every task, action, decision/switch/menu, and **loop** is drawn as an ELK-laid-out SVG — loops render as **nested container boxes** (matching Architect) and each task starts at its true entry node. A tab per dependency flow (common modules, in-queue, transfer/bot targets — any type, incl. outbound) is discovered by matching referenced flow names to the org's flow list. High/mid/low detail levels, dark/light/white themes, fullscreen, a **variable & dependency cross-reference search** (click a variable to jump to where it's set/used, or a dependency to open its flow), node-level details (values set in that node, conditions/expressions, referenced flow/task, variables used), and **SVG / PNG / PDF / HTML / JSON download**. Read-only. The SPA calls the internal `POST /api/flow-yaml`, which forwards to the onboarding runner's `POST /api/export-yaml` (SDK export, shared-secret `x-export-key`). New SWA app settings: `RUNNER_BASE_URL` + `EXPORT_YAML_KEY`. Access key: `deployment.flowoverview`.
...
│   │   │   └── flowOverview.js      Flow Overview — interactive SVG overview of a flow + dependencies (ELK layout, nested loops)
│   │   ├── divisions/
