 ...
 *   own row is "customer-manager"
 *
 * The internal org has no scope; asking for one is a 400. Features are
 * validated against the pages a customer may hold (customerPages.js); the
 * rest are dropped and named in the response. Every PUT is written to the
 * activity log with the before and after lists, under the caller's verified
 * identity.
 *
...
/** Which org this caller may act on, or a refusal. */
function resolveOrg(context, caller, requested) {
  if (caller.mode === "customer") {
    ...
    return { customerId: caller.customerId };            // never what they sent
  }
  if (caller.mode !== "internal") return { error: "internal_only", status: 403 };
  if (!caller.superuser && caller.role !== "customer-manager") return { error: "customer_manager_required", status: 403 };
  const customerId = String(requested || "").trim();
  if (!customerId) return { error: "customerId_required", status: 400 };
  if (customerId === INTERNAL_ORG_SLUG) return { error: "internal_org_has_no_scope", status: 400 };
...
module.exports = async function (context, req) {
  try {
    ...
    if (method === "PUT") {
      const { kept, dropped } = filterCustomerPages(body.features);
