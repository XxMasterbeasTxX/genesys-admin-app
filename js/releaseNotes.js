...
export const RELEASE_NOTES = [
  ...
  },
  {
    ...
    changes: [
      ...
      "Two optional filters narrow who a run covers: Groups — every group type, social as well as official — and Division. Leave them empty to cover the whole org. The two combine: picking a group and a division gives the members of that group who are also in that division, not everyone in either.",
      ...
      "Users who already have a WebRTC phone are found up front rather than by a create that fails, so the run no longer sends a request for every user who is already set up. Finding them takes some care: the phone list Genesys returns does not always say which user a phone belongs to, so phones the list does not account for are read individually. A phone counts whether it is recorded as the user's WebRTC user or their owner, and phones on any WebRTC base count — not only the base new phones are created on.",
      ...
      "The Findings panel also lists every licence found in the org and which ones were read as collaborate, so if an org uses a licence name the tool does not recognise you can see it in the review rather than after the fact.",
      ...
      "The site picker is labelled Destination Site.",
