...
export const RELEASE_NOTES = [
  ...
  },
  {
    ...
    changes: [
      "New Flows › Flow Overview: an interactive, read-only graphical overview of an Architect callflow and all of its dependency flows. Pick an org and a live flow — with a type filter beside the picker for narrowing a long list — and the whole call flow is drawn as a diagram.",
      ...
      "Messaging, email and bot flows are drawn properly. A flow's top level is grouped differently per type — states for messaging and email, bots for bot flows, tasks everywhere — and only tasks were read, so everything else was skipped along with all of its contents, leaving a part-empty diagram and the start marker on the wrong box. Change State is drawn to the state it moves to, and Exit Bot Flow ends its path instead of running on.",
      "Call Bot Flow and Call Digital Bot Flow register the bot flow they call as a dependency, so it shows up in the diagram, the search panel, the dependency export and its own tab.",
      "Ask for Intent is drawn as the branch point it is — one labelled connection per intent, going wherever that intent routes — instead of a dead-end Collect Input, and a bot's Anything Else loop is drawn as a loop with its body nested inside. Handlers that are switched off, such as an unused Max No Inputs, no longer appear as paths the flow can take.",
