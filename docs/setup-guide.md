...
| 57a | Divisions — Routing — Skills | Same layout; loads ACD skills; Current Division populated and source-division dropdown offers the divisions in use. Moves via `PATCH /api/v2/routing/skills/{id}`, not the bulk division endpoint, which rejects skills (PUT answers 405 on that path) |
