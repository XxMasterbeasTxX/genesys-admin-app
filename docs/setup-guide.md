...
| 89 | Direct Routing — Add user(s) | Select org; pick users; click Load Details; users with work phone or email shown (others skipped); collapsible Addresses section per card; Primary and DR radios deselectable; email domain warning for unconfigured domains; Backup Settings (None/User/Queue + Wait for Agent); change only 1 user → Apply updates only that user; progress bar and summary |
