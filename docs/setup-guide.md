...
- **Roles — Create** — Create a new authorization role with a full permission builder. Domain and entity are selected via searchable comboboxes; action checkboxes cascade from the entity. Policy rows are grouped by domain under collapsible accordion sections (amber headings, all expanded by default). Optional **Conditions** panel per row (QUEUE_ID, MEDIA_TYPE, SKILL_ID, DIVISION_ID; operator INCLUDES/EQUALS; multi-select values). Submit POSTs via `PUT /api/v2/authorization/roles`. Access key: `roles.create`.
- **Roles — Edit** — Edit an existing role. Searchable combobox loads all roles; selecting one pre-fills the permission builder (wildcard `*` entities and actions are expanded against the catalog). Domain sections are collapsed by default in edit mode. Conditions are pre-populated from `resourceConditionNode`. Save submits `PUT /api/v2/authorization/roles/{id}`. Access key: `roles.edit`.
