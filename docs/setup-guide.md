...
- **Data Tables — Edit** — Edit an existing data table in the selected org. Select a data table from a dropdown, click **Load** to fetch the full table with schema. Form pre-populated with Name, Division, Description, Key (read-only), and schema columns sorted by `displayOrder`. Columns are draggable; on save, `displayOrder` is assigned by DOM position. Save via `PUT /api/v2/flows/datatables/{id}`. Access key: `data-tables.edit`.
...
- **Skill/Role/Queue Templates Export** — Export selected skill templates to a multi-sheet Excel workbook. Templates are loaded from Azure Table Storage; the user picks one or more via checkboxes. Seven sheets: **Overview** (template name, role/skill/language/queue counts, user/group/team assignment counts, schedule count), **Roles** (one row per template × role × division), **Skills** (template, skill, proficiency), **Languages** (template, language, proficiency), **Queues** (template, queue), **Members** (template, type — User/Group/Work Team, name, assigned by), **Schedules** (template, mode, schedule type, time, day/date, enabled, targets, last run, last run status, created by). Preview table, download, email, and schedule panel with dynamic template picker. Supports per-org scheduled automation with template selection stored in `exportConfig`. Access key: `export.users.skillTemplates`.
...
| 91 | Data Tables — Edit | Select org; data tables loaded in dropdown; select table and click Load; form pre-filled with Name, Division, Description, Key (read-only), schema columns; drag to reorder; add/remove columns; Save updates via PUT; Activity Log entry |
