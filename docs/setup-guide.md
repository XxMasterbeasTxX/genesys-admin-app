...
- **Roles — Permissions vs. Users** — Three modes selectable via a top toggle:
  ...
  - **Hourly Interacting** — Analyse which users hold the `billing:user:hourlyInteracting` permission and whether they are eligible for the Hourly Interacting license or require a Full CX license. Fetches the current list of disqualifying permissions from `GET /api/scrape-disqualifying-permissions` (with static fallback). Classifies users as **Hourly** (no disqualifying permissions) or **Full CX** (one result row per billing-role × forbidden-role). Unique user counts in status pills. Export to Excel. Access key: `roles.search`.
  - **WEM License** — Find every user whose roles carry a permission that triggers a Workforce Engagement Management add-on licence, and cross-check that against the licences they actually hold. Nothing is scraped: Genesys publishes no list of WEM-triggering permissions, so `GET /api/v2/license/definitions` supplies the mapping (per org) and `POST /api/v2/license/infer` — body `[roleId]`, the same inference the Genesys admin UI runs — decides whether a role triggers. The page identifies the WEM add-on itself, on a `/wem/i` hint against the licence id and description, and states what it is checking — “Checking against **gc2WEMupgrade**.” There is no picker: the page answers one question, and choosing a licence would need the one thing an admin has no reason to know. An org on `cloudCX3` or above has **no WEM SKU to tick** — CX3 bundles WEM, and `/license/definitions` returns only what an org can hold — so the page names the bundling and says nobody there can be unlicensed for WEM, instead of asking for a selection that does not exist. Users are classified **Unlicensed trigger** (triggers, no WEM licence assigned), **Licensed**, or **License unused** (holds WEM, no role needs it, from `GET /api/v2/license/users`). One row per user × triggering role, with source attribution and the permissions responsible. Group memberships come from the bulk user fetch (`expand=authorization,groups`), so no per-user request is made. If `/license/infer` is unavailable the page falls back to matching permissions against the licence definition and says so on screen. Export to Excel carries the full permission list. Access key: `roles.search`.
...
| 86 | Roles — Permissions vs. Users — Hourly Interacting mode | Toggle to Hourly Interacting; users with `billing:user:hourlyInteracting` listed; Hourly vs Full CX classification; disqualifying permissions shown; unique user counts in pills; export to Excel |
