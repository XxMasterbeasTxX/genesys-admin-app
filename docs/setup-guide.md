...
- **Configure Users** — Assign roles, skills, language skills, and queue memberships to one or more users at once. Two-panel layout: left panel for user selection (modes: Search, By Group, By Role, Reports To, Location, By Division), right panel for configuration. Apply button at the top. Five collapsible sections: Templates (multi-select — apply one or more saved templates), Roles (with per-role division picker), Skills (with proficiency 1–5), Language Skills (with proficiency 1–5), and Queues. Template and manual items are merged additively. Progress bar and per-user success/error log during execution. Access key: `users.rolesSkills.configureUsers`.
...
| 96 | Configure Users | Select org; choose user selection mode (Search/By Group/By Role/Reports To/Location/By Division); select users in multi-select; right panel: optionally select one or more templates (multi-select); add individual roles with per-role divisions, skills with proficiency, language skills with proficiency, queues; Apply button at top; confirm dialog; progress bar + per-user log |
