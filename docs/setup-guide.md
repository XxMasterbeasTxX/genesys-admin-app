...
- **WebRTC Phones — Create** — Review who needs a WebRTC phone (by licence and existing phones), then bulk-create for the confirmed selection, with Excel log export
...
| 18 | WebRTC Phones — Create | Site selector; Analyse lists who is in scope and who is skipped with the reason; ticking/unticking changes the create count; confirmation dialog appears; create runs and fills the Result column; Excel download works |
