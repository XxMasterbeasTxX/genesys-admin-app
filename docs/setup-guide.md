...
| 98 | Utilities — Get Lists — Permissions vs. Licenses | Pick the list and Load; one row per permission with Permission / Label / Domain / Entity / Action / Division Aware / Conditions / # Licences, then one column per licence the org holds, sorted by permission string; a permission in no licence shows 0 and no ticks; a permission in several ticks each (e.g. `quality:evaluation:add` across cloudCX2 and the WEM upgrades); column filters and click-to-sort work on the licence columns; Export writes the visible rows with the licence columns; switching to another list clears the table and its columns; the list is absent from the picker without `authorization:license:view`, and lacking it does not hide the other lists |
