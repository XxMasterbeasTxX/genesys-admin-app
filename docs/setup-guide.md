...
- **Skill Templates — Create Template** — Create reusable templates of roles (with per-role division access), skills (with proficiency levels 1–5), and queues for bulk user provisioning. Templates are stored in Azure Table Storage (`skilltemplates` table), not in Genesys (which has no native template concept). Two-panel page: left panel lists all templates for the selected org (columns: Name, Roles, Skills, Queues, Created By, Actions); right panel is an inline editor with three collapsible sections (Roles, Skills, Queues). Roles section shows a role card per added role, each with an embedded division multi-select. Skills section has a searchable multi-select plus per-skill proficiency radio buttons (1–5, default 3). Queues section has a searchable multi-select. Full CRUD: create, edit (owner or admin only), delete (owner or admin only). Data is partitioned by org. Access key: `users.rolesSkills.createTemplate`.
...
| 95 | Skill Templates — Create Template | Select org; template list loads; click Create Template; collapsible Roles/Skills/Queues sections; add roles with per-role division selection; add skills with proficiency (1–5); add queues; save template; edit/delete restricted to owner + admin |
