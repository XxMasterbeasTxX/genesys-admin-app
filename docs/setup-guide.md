...
- **Data Tables — Copy (Single Org)** — Copy a data table (structure + optionally rows) within the same org, with division selection. After choosing a source table, an editable schema column list (Name / Type / Default, drag-to-reorder) lets you add or remove columns before saving; the primary key column is preserved. Copied rows are filtered to the kept columns (removed columns dropped, added columns take their default).
