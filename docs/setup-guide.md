...
- Added **Roles › Create → Templates** — Clean Admin and one clean role per licence the org holds, computed from `/license/definitions` and verified with `/license/infer` after creation.
- Added **Roles — Permissions vs. Users › STA License** mode — the WEM check for the Speech and Text Analytics add-on. Both tabs are now descriptors over a shared `addonLicense.js`; the extraction was behaviour-preserving and WEM is unchanged. Gated on `roles.search` action `sta`.
- Added **Utilities — Get Lists** page at nav path **Utilities > Get Lists** — a registry-driven viewer for the small reference lists in an org. A **List** dropdown picks which one; the result is shown with per-column filters and click-to-sort headers, and exports to Excel. Read-only. Lists: **Presence Definitions** (`GET /api/v2/presence/definitions`, `localeCode=ALL`), flattened to one row per language label, and **Wrap-up Codes** (`GET /api/v2/routing/wrapupcodes`, paginated) with Name, ID, Description and Division Name, the division name filled in from `GET /api/v2/authorization/divisions` when the embedded reference omits it. **Permissions vs. Licenses** joins the permission catalog against the inverted `permissions.ids` of every licence definition — **one row per permission-licence pair**, so filtering Licence to one value gives that licence's permissions; no per-permission calls. Shows what each licence *grants* rather than what triggers a charge. Adding a further list is one entry in `LIST_DEFS` in `js/pages/utilities/getLists.js`, and each entry names an `action` so it is **gated on its own permission**; a list the user cannot read is shown disabled in the picker with the permission it needs, rather than hidden. Access key `utilities.getLists`.
...
- **Utilities — Get Lists** — Reference-list viewer for the selected org. A **List** dropdown chooses which list and a **Load** button beside it fetches it — nothing loads automatically, and changing the picker clears back to idle without issuing a request; results render in a table with a per-column filter dropdown in each heading cell (single header line), each value offering an **only** action for one-click narrowing, plus value search, All/None, a visible/total count, and click-to-sort headers (sorting reorders rows in place so filters survive). **Export to Excel** writes the visible rows in the displayed order. Read-only. Driven by a `LIST_DEFS` registry in `js/pages/utilities/getLists.js` — one entry per list supplying label, description, columns and a fetch function — so further lists need no new page. Currently: **Presence Definitions** (`GET /api/v2/presence/definitions` with `localeCode=ALL`, not paginated), flattened to one row per language label, with a second best-effort `deactivated=TRUE` call merged by id. Columns: System Presence, Language, Label, Type, Deactivated, ID, Division ID. Access key: `utilities.getLists`.
...
- **Data Tables — Create** — Create a new data table in the selected org. Required fields: Name, Division (loaded from the org), Key (display name of the primary key — always stored as `string`). Optional: Description and additional schema columns. Each schema column has a Name, Type (Boolean / Decimal / Integer / String), and optional default value. Columns can be reordered by dragging the grip handle. Schema can be imported from an Excel file: click **Import from Excel**, select a `.xlsx`/`.xls` file, choose the sheet, and click **Load**. Fixed format: row 1 = `Name` / table name; row 2 = `key` / key name; row 3 = `division` / division name; row 4 = `description` / description text; rows 5+ = A=column name, B=type, C=default value (optional). A **Download Template** button downloads the pre-formatted Excel template directly.
- **Data Tables — Edit** — Edit an existing data table in the selected org with a two-mode editor (**Schema** / **Rows**). Select a table from the dropdown to auto-load (no separate Load button). Schema mode supports drag reorder, add/remove/modify columns, and save via `PUT /api/v2/flows/datatables/{id}`. Rows mode supports bulk row editing with full-table search, pagination (50/100/200), Add Row, row checkboxes, Select-all-on-page, Delete Selected, Undo All, Refresh Rows, and Save Changes with per-row status. Row persistence uses `POST`/`PUT`/`DELETE` on table rows depending on row state. Access key: `data-tables.edit`.
- **Data Tables — Copy** — Copy a data table (structure + optionally rows) within the same org, with division selection. After choosing a source table, an editable schema column list (Name / Type / Default, drag-to-reorder) lets you add or remove columns before saving; the primary key column is preserved. Copied rows are filtered to the kept columns (removed columns dropped, added columns take their default).
- **Data Tables — Copy between Orgs** — Copy a data table (structure + optionally rows) from one customer org to another, with target division and optional row copy. The schema is shown as an editable column list (Name / Type / Default, drag-to-reorder) so columns can be added or removed before saving; the primary key column is preserved and copied rows are filtered to the kept columns.
...
- **Data Actions — Test** — Runs data actions and shows the result against the output contract; cannot change anything. Granted by `data-actions.test` (`integrations:action:execute` only), so testing can be given without edit or publish rights. Published and Draft can be run together with differing outputs marked. A test executes the action for real — the page names the HTTP method before running, and warns when Both executes it twice. Access key: `data-actions.test`.
- **Data Actions — Edit** — View, edit and test existing data actions with a draft/publish workflow. The org comes from the **header picker**; a searchable action dropdown is narrowed further by category, integration and status. **Contract, Configuration and Test** are three collapsible sections in the order the Genesys editor uses, all closed on open. Save Draft / Validate / Publish sit in a bar under the picker alongside the status line and an unsaved-changes marker. Editable: name, request config (method, URL template, body, headers), response config (translation map, defaults, success template), **Execution Timeout** (1–60s), and the input/output contract schemas of a **draft-only** action. **Read-only: category, integration, and a published action's contract** — Genesys fixes those, and the page says so rather than showing a box that does nothing. A test result is rendered against the action's own output contract (each output by name, type and value), with execution steps in their own table and the raw response collapsed. Access key: `data-actions.edit`.
...
- **Queues/Skills Export** — Export users with columns **Name, Queue, Skill, Language Skill**. Filters are split into Group 1 (**User, Groups, Work Teams**) and Group 2 (**Queues, Skills, Language Skills**): Group 1 selects candidate users, Group 2 filters row values. Logic is OR within each filter and AND across filters. One row per Queue × Skill × Language Skill combination with blanks for missing dimensions unless that dimension is filtered. Preview pagination supports 50/100/200 rows per page. Supports download, email, and per-org scheduled automation.
- **Queues/Skills Scheduled Filters** — The schedule form supports optional filters for Users, Groups, Work Teams, Queues, Skills, and Language Skills. Scheduled runs apply the same Group 1/Group 2 filtering semantics as manual runs. No selected filters means export all users in the selected org.
...
- **Billing — Single Org** — Export the latest complete billing period for a single trustor org to a styled Excel workbook. Resolves the org's trustee customer via `customers.json::trusteeForOrg`, fetches the trustor org ID with `GET /api/v2/organizations/me`, then calls `GET /api/v2/billing/trusteebillingoverview/{trustorOrgId}?billingPeriodIndex=N`. Period dropdown: Current (in-progress) / Latest complete (default) / Two / Three periods ago. Supports email and per-org scheduled automation (`exportType: "billingSingleOrg"`, handler `api/lib/exports/billingSingleOrg.js`). Access key: `export.billing.singleOrg`.
- **Billing — All Orgs (Latest)** — Export the latest complete billing period for every billable customer org in a single workbook (one sheet per org, sheet name truncated to 31 chars). Supports email and scheduled automation (`exportType: "billingAllOrgsLatest"`, handler `api/lib/exports/billingAllOrgsLatest.js`). Access key: `export.billing.allOrgsLatest`.
- **Billing — Calendar Year** — Export 12 months of billing data per org for a chosen calendar year. Per-org sheet stacks all matched periods vertically with a metadata banner + `Completed Periods` count, per-period blue divider, green summary subheader, and license rows. Scheduled variant (`exportType: "billingCalendarYear"`, handler `api/lib/exports/billingCalendarYear.js`) always exports the **previous** calendar year (current year − 1). Access key: `export.billing.calendarYear`.
- **Billing — Date Range** — Export billing data for a custom month range (from-month/year to to-month/year, default last 3 completed months) across all billable orgs. Periods are included if they overlap `[fromDate, toDate]`; walk stops after 2 consecutive periods entirely before `fromDate`. Per-org sheet has a top metadata banner with the range label and per-period blue divider + green `─── BILLING SUMMARY ───` subheader. Browser only — no scheduled variant. Access key: `export.billing.dateRange`.
- **Billing — Custom Orgs** — Export a single billing period (Current / Latest / 2 ago / 3 ago) for a user-picked subset of orgs (checkbox list with select-all / deselect-all controls and a live `(N selected)` counter; one sheet per selected org). Browser only — no scheduled variant. Access key: `export.billing.customOrgs`.
- **Billing — Period Comparison** — Side-by-side comparison of 2–4 billing periods for a single org, with variance Δ / % columns between adjacent periods (green for positive, red for negative). Auto-sorted chronologically. Re-uses the cached billing overview from `fetchBillingPeriods` (no extra API calls). Unique single-sheet layout: merged title banner, generated timestamp, per-period header (Prepay Qty / Usage Qty / Overage Qty), per-gap variance header (Δ Absolute / % Percentage). Variance: `Δ = next.usage − cur.usage`; `% = Δ/cur` (or `±9999%` when `cur=0` and `Δ ≠ 0`). Browser only — no scheduled variant. Access key: `export.billing.periodComparison`.
- **Roles — Compare** — Three modes selectable via a top toggle:
  - **Compare Roles** — Compare permission policies across 2–10 roles from the same org side by side. Roles are loaded on page load; select any combination and click Compare to fetch each role's full permission set in parallel via `GET /api/v2/authorization/roles/{id}`. Wildcard permissions (`*` entity or `*` action) are automatically expanded against the full Genesys permission catalog (`GET /api/v2/authorization/permissions`) so all concrete permissions are visible. Results are grouped by domain (collapsible, all collapsed by default) with a permission matrix: Entity column + one column per role showing action tags or `—`. Rows are colour-coded: amber = differs across roles, green = identical. Toggle between All permissions and Differences only. Filter, Expand/Collapse all. Export to Excel.
  - **Compare Users** — Compare the effective permissions of exactly 2 users side by side. Users are found via a search-as-you-type picker (`POST /api/v2/users/search`, CONTAINS on name + email). For each user, direct role assignments are fetched via `GET /api/v2/authorization/subjects/{userId}` and group memberships via `GET /api/v2/users/{userId}?expand=groups`; each group's roles are fetched via `GET /api/v2/authorization/subjects/{groupId}`. Group names are resolved via `GET /api/v2/groups/{groupId}`. All role permissions are unioned per user with full attribution: each permission cell shows the role name and whether the role was **Assigned manually** or **Inherited from Group: GroupName**. Roles granted by a group are never also labelled as manually assigned. Defaults to Differences only view. Export to Excel includes an extra `ColName — via roles` column per user showing attribution. Column names are disambiguated with `(A)`/`(B)` suffixes when both users share the same display name.
...
- **Roles — Permissions vs. Users** — Three modes selectable via a top toggle:
  - **Permission Search** — Search for all users who hold a specific permission in the org. Select a domain, entity, and one or more actions from the full permission catalog. On search: (1) the catalog is used to find all roles that carry the permission (`GET /api/v2/authorization/roles?permission={domain}:{entity}:{action}` + client-side wildcard filter); (2) all org users are fetched with `expand=authorization` to cross-reference membership; (3) for each matching user the source is resolved asynchronously — direct assignment vs. inherited via group (batches of 10). Results stream into a table as they resolve. Trustee-org users are excluded. Client-side action-filter chips narrow results after load. Progress bar tracks Steps 1, 2b, and 3. Export to Excel with filename `Roles_Search_{Org}_{Domain}_{Entity}_{Actions}_{timestamp}.xlsx`. Access key: `roles.search`.
  - **Hourly Interacting** — Analyse which users hold the `billing:user:hourlyInteracting` permission and whether they are eligible for the Hourly Interacting license or require a Full CX license. Fetches the current list of disqualifying permissions from `GET /api/scrape-disqualifying-permissions` (with static fallback). Classifies users as **Hourly** (no disqualifying permissions) or **Full CX** (one result row per billing-role × forbidden-role). Unique user counts in status pills. Group memberships arrive with the bulk user fetch rather than one request per matched user. Export to Excel writes only the visible rows, so filters narrow it too. Access key: `roles.search`.
  - **WEM License** — Find every user whose roles carry a permission that triggers a Workforce Engagement Management add-on licence, and whether a WEM licence is also assigned. Nothing is scraped: `POST /api/v2/license/infer` decides whether a role triggers (the same inference the Genesys admin UI runs), and `GET /api/v2/license/definitions` only names which permissions were responsible — each licence netted against its own prerequisites, so on an org holding two WEM SKUs one licence's base cannot cancel the other's triggers. The page identifies the add-on itself on a `/wem/i` hint and states it read-only (“Checking against **gc2WEMupgrade**”); there is no picker. Tiers need no special handling because the permission set comes from whichever SKU the org holds; an org on `cloudCX3`+ has none, since CX3 bundles WEM, and the page says so and disables Search. Two categories — **Unlicensed trigger** and **Licensed** — with no third “licence but no trigger” case, which cannot occur. The page makes no claim about cost (named vs. concurrent licensing mean different things) and says so under the results. A **Roles** view groups the same findings by role, ordered by how many users each affects. Group memberships come from the bulk user fetch, so no per-user request is made. If `/license/infer` is unavailable the page falls back to permission-matching and says so. Export follows the view on screen. Access key: `roles.search`.
  - **STA License** — The same question as WEM, for the Speech and Text Analytics add-on: who holds a permission triggering `gcSTAupgrade`, via which role, and whether an STA licence is assigned. Shares its engine with the WEM tab, so classifier, prerequisite subtraction, source attribution, Roles view and export behave identically. Standard on CX 1 and CX 2, bundled into CX 3 — a CX 3 org says so and disables Search. WEM outranks STA and `POST /api/v2/license/infer` applies that precedence itself, so on an org holding both the tab correctly finds nobody and names the licence that took precedence rather than showing an empty table. Access key: `roles.search` (action `sta`).
- **Roles — Create** — Create a new authorization role with a full permission builder. Domain and entity are selected via searchable comboboxes; action checkboxes cascade from the entity. The domain stays selected after clicking **Add** so more entities can be added without re-selecting. **Add All Entities** adds every entity in the selected domain at once with all their actions. Policy rows are grouped by domain under collapsible accordion sections (amber headings, all expanded by default). Each row has an inline **✎ edit** button to modify its action set without removing and re-adding. Optional **Conditions** panel per row (QUEUE_ID, MEDIA_TYPE, SKILL_ID, DIVISION_ID; operator INCLUDES/EQUALS; multi-select values). Submit creates a new role via `POST /api/v2/authorization/roles`. Access key: `roles.create`.
- **Roles — Create → Templates** — A **Templates** dropdown pre-filling the builder with a role that stops at what the org already pays for: **Clean Admin** (base licences + everything needing no licence) and one **Clean `<licence>`** per add-on the org holds, listed from `/license/definitions` so it mirrors the Licence column in Permissions vs. Licenses. A licence template drops permissions another add-on also grants, since those could invoke that licence instead — Clean STA is 4 of 23 on an org holding WEM, which is named as holding the rest. The set loads into the ordinary builder so anything can be removed before creating, and the created role is verified with `POST /api/v2/license/infer` and the verdict shown. Access key: `roles.create`.
- **Roles — Edit** — Edit an existing role. Searchable combobox loads all roles; selecting one pre-fills the permission builder (wildcard `*` entities and actions are expanded against the catalog). The domain stays selected after clicking **Add** so more entities can be added without re-selecting. **Add All Entities** adds all entities for the selected domain at once. Domain sections are collapsed by default in edit mode. Each row has an inline **✎ edit** button to modify its action set without removing and re-adding. Conditions are pre-populated from `resourceConditionNode`. Save submits `PUT /api/v2/authorization/roles/{id}`. Access key: `roles.edit`.
- **Roles — Copy** — Copy a role within the same org. Select a source role from a combobox; the name is pre-filled as "Copy of {name}" and the description and all permissions are loaded into the full permission builder for review. Name and description are editable before submitting. Optional **Make Hourly Interacting** checkbox: when checked, the created role has all disqualifying permissions stripped and `billing:user:hourlyInteracting` added; a collapsible post-creation summary lists removed and added permissions. Submit posts a new role via `POST /api/v2/authorization/roles`. Access key: `roles.copy.singleOrg`.
...
- **Template Schedules** — Automate template application to users, groups, and work teams on a precise schedule using Azure Durable Functions. Two modes: **Reset** (wipe ALL existing skills, languages, and queue memberships, then re-apply only the template — roles untouched) and **Add** (additive merge). Schedule types: One-time, Daily, Weekly, Monthly. Inline schedule panel on Create Template page (🕐 button) with a **Targets** section: search and add individual users (by name/email), groups (dropdown), or work teams (dropdown) — selected targets are displayed as removable chips with type labels (U/G/WT) and stored as a JSON array in the schedule. At least one target is required. Group and work team members are resolved live from the Genesys API at execution time, so membership changes are automatically picked up. Dedicated Template Schedules overview page with targets summary column. Durable Functions orchestrator sleeps via `createTimer(exactDateTime)` for precise execution — no polling. All times in Danish time (Europe/Copenhagen). Access key: `users.rolesSkills.templateSchedules`.
...
- **Direct Routing — Add user(s)** — Assign the `directrouting` integration tag to a user's work phone (1–3) or email address, attach the number to a call route, and configure agent-level backup routing. Multi-select user picker with an optional group filter. Only users the page can act on are listed — a work phone in a **DID pool**, or an email on a **configured inbound domain**; a tag on anything else routes nothing. One tag per media type, set with a toggle. Integration column shows existing tags and they are never cleared by this page unless it set them. Primary is read-only (derived by Genesys, and not recommended for direct routing). Call Routing column assigns the number to an Architect IVR, applied grouped by route. Backup supports a user and a queue together, with *Send to backup immediately* and a 60–864000s wait. Change detection — only modified users are submitted. Addresses, backup and call routes are gated by separate Genesys permissions. Design: [direct-routing-add-users-design.md](direct-routing-add-users-design.md). Access key: `users.directRouting.add`.
...
- **Configure Users** — Assign or remove roles, skills, language skills, and queue memberships for one or more users at once. Two-panel layout: left panel for user selection (modes: Search, By Group, By Role, Reports To, Location, By Division), right panel for configuration with an Add/Remove mode toggle. Users appear as expandable rows with checkboxes — each row shows the user's name, email, template tags (from tracked assignments), and count badges for roles, skills, languages, and queues. Click a row to expand and view full assignment details (roles with divisions, skills with proficiency, languages with proficiency, queue memberships) — role grants and queues are lazy-loaded on expand. Select-all checkbox at the top. Right panel: Add/Remove radio toggle, Apply button, and five collapsible sections: Templates (multi-select), Roles (with per-role division picker in add mode), Skills (with proficiency 1–5 in add mode), Language Skills (with proficiency 1–5 in add mode), and Queues. In Add mode, each selected role must have at least one division selected before Apply is enabled. Template and manual items are merged additively; template assignments are recorded in Azure Table Storage (`templateassignments` table). In Remove mode: removing a template cascade-removes all associated roles, skills, languages, and queue memberships — a confirmation dialog lists every property that will be removed before proceeding. Progress bar and per-user success/error log during execution. Access key: `users.rolesSkills.configureUsers`.
- **Data Tables — Create** — Create a new data table in the selected org. Required fields: Name, Division (loaded from the org), Key (display name of the primary key — always stored as `string`). Optional: Description and additional schema columns. Each schema column has a Name, Type (Boolean / Decimal / Integer / String), and optional default value. Columns can be reordered by dragging the grip handle. Schema can be imported from an Excel file: click **Import from Excel**, select a `.xlsx`/`.xls` file, choose the sheet, and click **Load**. Fixed format: row 1 = `Name` / table name; row 2 = `key` / key name; row 3 = `division` / division name; row 4 = `description` / description text; rows 5+ = A=column name, B=type, C=default value (optional — empty or invalid values are silently skipped; boolean accepts `true`/`false`; integer must be a whole number; decimal must be numeric; string accepts any non-empty value). Division is matched case-insensitively (skipped if not found). Multiple tabs can be imported from the same file without re-selecting it. A **Download Template** button downloads the pre-formatted Excel template directly.
- **Deployment — Basic** — Bulk-create core Genesys Cloud objects from a single Excel workbook. Select a `.xlsx`/`.xls` file; each sheet is matched by tab name to a specific object type and processed automatically. Supported tabs: **DID Pools** (A=Number Start E.164, B=Number End, C=Description, D=Comment, E=Provider: PURE_CLOUD_VOICE / BYOC_CLOUD / BYOC_PREMISES — skipped if overlapping pool exists); **Divisions** (A=Name, B=Description — skipped if name exists); **Sites** (A=Name, B=Media Model Cloud/Premises, C=Media Regions comma-sep for Cloud, D=Location Name, E=TURN Relay Site/Geo, F=Caller ID, G=Caller Name, H=Description — skipped if name exists); **Skills** (A=Name — skipped if exists); **Skills - Language** (A=Name — skipped if exists); **Site - Number Plans** (A=Site Name, B=Plan Name, C=Classification, D=Match Type: numberList/digitLength/intraCountryCode/interCountryCode/regex, E=Priority, F=State, G=Numbers one per row for multi-number types, H=Digit Length e.g. 4-10, I=Match Pattern, J=Normalized Format — GET→merge→PUT per site, preserving existing plans); **Site - Outbound Routes** (A=Site Name, B=Route Name, C=Classification Types one per row, D=Distribution: SEQUENTIAL/RANDOM, E=Trunk Names one per row resolved by name, F=State true/false — existing routes not in sheet are untouched; routes matched by name are updated, new ones created); **Schedules** (A=Name req, B=Division, C=Description, D=Start req ISO-8601 no-tz e.g. `2026-01-01T08:00:00.000`, E=End req same format, F=RRule optional iCal string e.g. `FREQ=WEEKLY;BYDAY=MO,TU,WE,TH,FR` — times interpreted in org timezone; existing schedules matched by name are updated); **Schedule Groups** (multi-row per group — rows sharing the same Group Name are folded into one upsert: A=Group Name req, B=Division first-row-only, C=Description first-row-only, D=Time Zone first-row-only validated against Genesys allowed values e.g. `Europe/Copenhagen`, E=Type req: `open`/`closed`/`holiday`, F=Schedule Name req resolved by name — invalid timezone surfaced as a clear error before the API call); **Users** (multi-row per user — rows sharing the same E-mail are folded into one upsert: A=User display name req for new, B=E-mail req, C=Phone Name, D=Phone Site, E=Division, F=Skill one per row, G=Role one per row, H=Extension, I=DID Direct Number, J=Phone Type exact base-settings name, K=Queue one per row — upserts user by email; restores deleted/inactive users to active before other steps; grants roles via `POST /api/v2/authorization/roles/{roleId}` per role; adds routing skills; creates phone if not found; sets extension and DID; adds users to queues in bulk after all users are processed; per-step failures are warnings not fatal); **Wrapup Codes** (A=Name req, B=Division, C=Description, D=Queue Name — if set, assigns the code to that queue after upsert; existing codes matched by name are updated); **Queues** (A=Queue Name req, B=Division req, C=Description, D=Scoring Method: TimestampAndPriority/PriorityOnly, E=Last Agent Routing: Disabled/QueueMembersOnly/AnyAgent, F=ACW Prompt: OPTIONAL/MANDATORY/MANDATORY_TIMEOUT/MANDATORY_FORCED_TIMEOUT/AGENT_REQUESTED, G=Skill Eval Method: NONE/BEST/ALL, H=Enable Transcription, I=Enable Manual Assignment, J=Suppress Recording, K=Calling Party Name, L=Calling Party Number, M=Call In-Queue Flow, N=Email In-Queue Flow, O=Message In-Queue Flow, P=Call Script, Q=Callback Script, R=Chat Script, S=Email Script, T=Message Script, U–Z=Call media: Alerting/AutoAnswer/AutoAnswerAlertTone(s)/ManualAnswerAlertTone(s)/SL%/SLDurationMs, AA–AF=Callback media, AG–AL=Chat media, AM–AR=Email media, AS–AX=Message media — blank cells are omitted; only Queue Name and Division are required; invalid non-blank values skip the row). Row 1 per sheet is always a header and is skipped. Results shown per row (✓ created/updated/skipped / ✗ error) with a per-tab summary. All creations logged to the Activity Log with a `[Deployment]` prefix.
- **Deployment — Data Tables** — Bulk-create data tables from an Excel workbook in a single click. Click **Select Excel Sheet**, pick a `.xlsx`/`.xls` file, and every sheet is automatically processed in order. Fixed row format per sheet: row 1 = `Name` / table name; row 2 = `key` / key name; row 3 = `division` / division name; row 4 = `description` / description text; rows 5+ = A=column name, B=type (`boolean`, `string`, `integer`, `decimal`), C=default value (optional — empty or invalid values silently skipped). Each sheet produces one data table; results are shown inline (✓ created / ✗ error per sheet) with a final summary. A **Download Template** button downloads the pre-formatted Excel template directly. Restricted to `deployment.datatables` access key. All creations are logged to the Activity Log with a `[Deployment]` prefix.
- **Audit — Search** — Query Genesys Cloud audit events across any date range. Ranges ≤ 14 days automatically query **all realtime-supported services** concurrently using the synchronous `POST /api/v2/audits/query/realtime` endpoint (no polling, cursor-paginated to retrieve all results) — results appear in seconds. For ≤ 14-day ranges with a specific service not supported by the realtime endpoint, falls back to the standard async query API automatically. Ranges > 14 days require a service selection and always use the async chunked pipeline (`POST /api/v2/audits/query` → poll → cursor-paginated results, 30-day chunks). Preset quick-filters: Today, Last 7 days, Last month, Last 3 months. Auto-runs today's query on page load with no service pre-selected (all services). Client-side filters: Entity Type → Action (cascading) + Changed By. Results table: Date & Time, Service, Entity Type, Entity Name (resolved via 40+ mapped API paths with `(deleted)` label on 404), Action, Changed By (user or OAuth client name). Click any row to expand a detail panel showing metadata, changed properties (old → new values), additional context, and a raw API response dump. Sticky table header, sortable latest-first, configurable rows per page (50/100/150/200). A blue/amber hint below the service dropdown indicates the current query mode. **Export to Excel** button exports all filtered results — one row per property change — with columns: Date & Time, Service, Entity Type, Entity Name, Action, Changed By, Level, Remote IP, Property, Old Value, New Value, Additional Context.
...
The Timer Function App also hosts the **Durable Functions** for template scheduling (starter, orchestrator, and activity functions). These are deployed alongside the timer trigger.

Deploy the `timer-functions/` folder to this Function App using Azure Functions Core Tools:
...
The SWA needs to notify the Timer Function App when template schedules are created, updated, or deleted:

```bash
az staticwebapp appsettings set --name genesys-admin-app \
  --resource-group rg-genesys-admin \
  --setting-names "TIMER_FUNCTION_URL=https://<your-timer-app>.azurewebsites.net"
```

| Setting | Value |
| --- | --- |
| `TIMER_FUNCTION_URL` | The Timer Function App URL (e.g. `https://genesys-admin-timer-xxx.swedencentral-01.azurewebsites.net`) |
...
#### Template Schedules (Durable Functions)

Template scheduling uses Azure Durable Functions for precise time-based execution:

1. When a template schedule is created or updated via the SWA API (`/api/template-schedules`), the API notifies the Timer Function App by POSTing to `/api/template-schedule-starter`
2. The starter function launches (or restarts) a Durable Functions orchestrator instance (`tplsched-{scheduleId}`)
3. The orchestrator computes the exact next fire time in Copenhagen timezone and sleeps via `context.df.createTimer(exactDateTime)` — no 5-minute polling
4. When the timer fires, the orchestrator calls an activity function that POSTs to `/api/template-runner` on the SWA
5. The template runner reads the schedule’s stored targets array (users, groups, work teams). User targets use the ID directly; group targets resolve members live via `GET /api/v2/groups/{id}/members`; work team targets resolve members live via `POST /api/v2/teams/{id}/members/query`. All resolved user IDs are deduplicated, then the template is applied in the configured mode:
   - **Reset**: Deletes all skills → deletes all languages → removes all queue memberships → re-applies template skills, languages, and queues. Roles are never touched.
   - **Add**: Bulk-patches skills, bulk-patches languages, and adds queue members additively.
6. For recurring schedules (Daily/Weekly/Monthly), the orchestrator calls `context.df.continueAsNew()` to loop. One-time schedules complete after a single execution.
...
| 13 | Data Tables — Copy | Tables, divisions load; copy structure + rows succeeds |
| 14 | Data Tables — Copy between Orgs | Source/dest selectors; copy with division + rows succeeds |
| 15 | Data Actions — Copy between Orgs | Source/dest selectors; searchable source action picker; integration mapping; copy succeeds; request/success templates preserved in destination |
| 16 | Data Actions — Edit | Pick org in header; search/filter actions; edit name, request/response config and Execution Timeout; draft-only action: also edit input/output contract schemas; save draft, validate, publish; test and read the outputs against the contract |
| 17 | Disconnect Interactions | Single/multiple/queue modes; media type + date filters; disconnect succeeds |
| 18 | WebRTC Phones — Create | Group and Division filters populate and narrow the scope (summary shows "N of M active users"); Destination Site selector; Analyse lists who is in scope and who is skipped with the reason; Findings reports how many existing WebRTC phones were matched — a zero there on an org with WebRTC phones means the match failed, do not create; ticking/unticking changes the create count; confirmation dialog appears; create runs and fills the Result column; Excel download works |
| 19 | WebRTC Phones — Change Site | Group and Division filters narrow the loaded phones (status shows "N of M at the site"); From/To selectors refuse the same site both sides; Load Phones; searchable multi-select; Move opens a confirmation before any write; run fills the summary; the picker clears afterwards so a stale list cannot be re-moved; Excel download works |
| 19b | WebRTC Phones — Delete | Visible to Master Admin, not the Admin group (confirm an Admin-group login does **not** see it); available to a customer login whose entitlements cover it; Site filter optional (no group/division filters — they match a holder, which these phones do not have), none = whole org; Analyse groups results as No user / Deleted user / Inactive user / Holder no longer exists / Could not be read; desk and conference phones never appear; phones held by active users are counted, not listed; "Could not be read" rows are locked and unselectable; inactive-user rows are listed unticked; Delete requires typing DELETE; a phone reassigned between Analyse and Delete is reported as Kept, not deleted; Excel download works |
...
| 37b | Skill Templates Export | Select org; load templates; pick ≥1 template via checkboxes; export runs; seven-sheet workbook (Overview, Roles, Skills, Languages, Queues, Members, Schedules); collapsible preview; download, email, schedule |
| 37c | Skill Templates email | Skill Templates export with email enabled sends attachment to recipients via Mailjet |
| 37d | Skill Templates scheduled export | Toggle automation; template picker loads dynamically per org; schedule saved with exportConfig.templateIds; Config column shown in schedule list |
...
| 43 | License Consumption Export | Page states it shows licences currently assigned and cannot match a concurrent bill; select org; click Load Licenses; licence definitions loaded into dropdown; choose "All Licenses" or a specific licence; export runs; collapsible preview with column filters on Name/Email/Division; Excel with Name, Email, Division + boolean licence columns; sheet "User Licenses" |
...
| 46a | Billing — Single Org | Select org; pick period (default Latest complete); click Run; styled Excel downloads with one sheet — metadata banner, Regular Licenses, AI Tokens Usage Breakdown (when present), Items with Overage |
| 46b | Billing — All Orgs (Latest) | Click Run; styled multi-sheet Excel downloads with one sheet per billable customer org |
| 46c | Billing — Calendar Year | Select year (default current); click Run; multi-sheet Excel with one sheet per org, all matched periods stacked |
| 46d | Billing — Date Range | Pick from-month/year and to-month/year; click Run; multi-sheet Excel with one sheet per org, periods overlapping the range stacked |
| 46e | Billing — Custom Orgs | Tick orgs (select-all / deselect-all available); pick single period; click Run; multi-sheet Excel with one sheet per selected org |
| 46f | Billing — Period Comparison | Select org; tick 2–4 periods; click Run; single-sheet Excel with periods side-by-side and Δ/% variance columns between adjacent periods (green/red highlighting) |
| 46g | Billing scheduled exports | Toggle automation on Single Org / All Orgs (Latest) / Calendar Year pages; Calendar Year scheduled runs always export the previous calendar year |
...
| 48 | GDPR — Request Status | Select org; click Load / Refresh; table shows Date, Type, Subject, Subject Type, Status, Completed, Details, Request ID; fulfilled Access requests show download links in Details column (downloaded via authenticated proxy); expired downloads show greyed-out “Expired” label with tooltip |
...
| 57a | Divisions — Routing — Skills | Same layout; loads ACD skills; Current Division populated and source-division dropdown offers the divisions in use. Moves via `PATCH /api/v2/routing/skills/{id}` with body `{ divisionId }`, not the bulk division endpoint, which rejects skills. Needs `routing:skill:update` |
...
| 78 | Deployment — Basic — DID Pools | Select a file with a "DID Pools" sheet; upload; rows created with correct start/end/provider; invalid rows report errors; Activity Log captures each creation |
| 79 | Deployment — Basic — Divisions | "Divisions" sheet; divisions created; duplicate names return API error per row; Activity Log captures each creation |
| 80 | Deployment — Basic — Sites | "Sites" sheet; Cloud site with comma-separated media regions and Geo TURN relay created correctly; `mediaRegionsUseLatencyBased` set per column E; Caller ID / Caller Name applied when columns F/G populated; Premises site skips media regions |
| 81 | Deployment — Basic — Skills | "Skills" sheet; routing skills created; Activity Log captures each creation |
| 82 | Deployment — Basic — Skills - Language | "Skills - Language" sheet; routing languages created; Activity Log captures each creation |
| 83 | Deployment — Basic — Site - Number Plans | "Site - Number Plans" sheet; plans grouped by site then plan name; GET existing plans first; sheet plans merged (overwrite by name, others preserved); PUT merged array; digitLength as integers; numbers omitted when empty; result shows one line per plan "Site — PlanName" |
| 84 | Deployment — Basic — Site - Outbound Routes | "Site - Outbound Routes" sheet; rows grouped by site then route name; classification types and trunk names accumulated across rows with same site+route; trunk names resolved to IDs via pre-fetched trunk base settings; GET existing routes per site; matching route by name → PUT update, new route → POST create; existing routes not in sheet untouched; result shows one line per route "Site — RouteName" |
| 85 | Deployment — Basic — Queues | "Queues" sheet; divisions/flows/scripts pre-fetched once; Queue Name and Division required (blank → skip); all other fields optional (blank → omit); invalid non-blank values skip row with reason; enum values case-sensitive; boolean fields accept true/false only; name→ID lookups for division, flows, scripts; media settings only included when at least one field in the block is populated; Activity Log captures each creation |
| 86 | Roles — Permissions vs. Users — Hourly Interacting mode | Toggle to Hourly Interacting; export writes only the visible rows, so a pill or text filter narrows it; users with `billing:user:hourlyInteracting` listed; Hourly vs Full CX classification; disqualifying permissions shown; unique user counts in pills; export to Excel |
| 87 | Copy Role — Make Hourly Interacting | Select a role; check "Make Hourly Interacting"; create; disqualifying permissions stripped; `billing:user:hourlyInteracting` added; collapsible summary shown |
| 88 | Copy Role (Between Orgs) — Make Hourly Interacting | Select source + target; select role; check "Make Hourly Interacting"; create in target; disqualifying permissions stripped; `billing:user:hourlyInteracting` added; collapsible summary shown |
| 89 | Direct Routing — Add user(s) | Select org; pick users (optionally narrowed by group — already-selected users stay selected, marked *(not in filter)*); Load Details; users with no DID-pool phone and no routable email are skipped with a count; Addresses section open by default; a number outside every DID pool has no toggle and states why; toggling one phone turns the others off; Integration column shows existing tags and a foreign tag survives Apply; Primary is read-only; Call Routing dropdown shows the current route and can change it — two users onto one route writes that route once; moving a number A→B reports 2 route updates; Backup with user + queue together shows (primary)/(secondary); wait under 60s is refused before Apply; Cancel works during Load Details; missing permissions grey the section, name the permission, and cannot be reached by keyboard |
| 90 | Data Tables — Create | Select org; click Create; form shown with Name, Division, Key, Description, schema columns; drag-to-reorder columns; Import from Excel; Download Template; Save creates table; Activity Log entry |
| 91 | Data Tables — Edit | Select org; data tables load in dropdown; selecting a table auto-loads schema+rows; toggle Schema/Rows; Schema: drag reorder/add/remove/save; Rows: full-table search, page size, Add Row, row checkboxes + select-all-page, Delete Selected, Undo All, Save Changes; Activity Log entry |
| 92 | Deployment — Basic | Select Excel file; each sheet processed by tab name; DID Pools, Divisions, Sites, Skills, Number Plans, Outbound Routes, Schedules, Schedule Groups, Users, Wrapup Codes, Queues; per-row results; Activity Log entries |
| 93 | Deployment — Data Tables | Select Excel file; each sheet creates one data table; per-sheet results; Download Template; Activity Log entries |
| 94 | Interaction Totals | Select org; pick date range + optional filters; click Load Totals; bar charts for Media Type, Voice Direction, ACD/Non-ACD Routing; uses Conversation Aggregates API (nConversations + hybrid interactionType/nOffered for routing); Export Excel with title rows; email with attachment; schedule automation with period preset |
| 95 | Skill Templates — Create Template | Select org; template list loads; click Create Template; collapsible Roles/Skills/Language Skills/Queues sections; add roles with per-role division selection; add skills with proficiency (1–5); add language skills with proficiency (1–5); add queues; save template; edit/delete restricted to owner + admin |
| 95a | Skill Templates — Schedule button | Click 🕐 on a template row; inline schedule panel opens; select mode (Reset/Add), type (One-time/Daily/Weekly/Monthly), time, day; add targets: search users by name/email, select groups from dropdown, select work teams from dropdown; targets shown as removable chips with type labels (U/G/WT); at least one target required; save schedule; badge count updates |
| 95b | Skill Templates — Add Users To Templates | Select org; template list loads; click template; right panel shows template details (roles, skills, languages, queues read-only); assigned users with remove buttons; user search (Search/By Group/By Division); select users via checkboxes; assign with confirm modal; progress bar |
| 95c | Template Schedules overview | Navigate to Template Schedules page; all schedules shown with template name, mode (Reset/Add), type, time, targets summary (U:/G:/WT: labels), enabled toggle, last run status; enable/disable toggle; delete button; schedules filtered by org |
| 96a | Copy from User | Select org; search source user; source card shows role/skill/language/queue counts; tick categories (Roles/Skills/Languages/Queues — all checked by default); search and add one or more target users (removable chips); preview tables (roles+divisions, skills+proficiency, languages+proficiency, queues); copy button resets each ticked category on every target (remove existing → apply source); progress bar, per-target log, cancel button |
| 96 | Configure Users | Select org; choose user selection mode (Search/By Group/By Role/Reports To/Location/By Division); users shown as expandable rows with checkboxes showing template tags and role/skill/language/queue counts; click row to expand detail view (lazy-loads role grants and queues); select-all checkbox; right panel: Add/Remove mode toggle; optionally select one or more templates (multi-select); add individual roles with per-role divisions (add mode only), skills with proficiency (add mode only), language skills with proficiency (add mode only), queues; Apply button at top; in remove mode, template removal shows warning listing all cascading removals + confirmation; progress bar + per-user log |
| 97 | Roles — Permissions vs. Users — WEM License mode | **Regression pass — the analysis now lives in the shared `addonLicense.js`, and every case below must behave exactly as it did before that extraction.** Toggle to WEM License; the page names the licence it identified ("Checking against gc2WEMupgrade") with Search enabled and nothing to tick anywhere; an org with no WEM add-on — whether because `cloudCX3` bundles it or because the org simply has none — says so, disables Search, and explains that nobody there can hold a WEM licence; results split into Unlicensed trigger / Licensed with unique user counts in pills, and no third category — a WEM licence is only ever held because a permission asked for it, so “licence with no trigger” cannot occur and such users are not listed; a user with no triggering role does not appear; triggering role, source (manual vs. group) and the responsible permissions are shown; permissions granted by the base licence are not listed as triggers, and the set comes from whichever WEM SKU the org holds so a CX 1 org reads `gc1WEMupgrade` and a CX 2 org `gc2WEMupgrade`; a caveat under the table states that what this costs depends on named vs concurrent licensing and the page claims neither; text filter and pills both narrow the table; a fourth **Roles** pill swaps to the same findings grouped by role (users / licensed / unlicensed / permissions, ordered by blast radius) with the filter matching role names; Export to Excel follows the view on screen and writes only the visible rows with the full permission list; with `/api/v2/license/infer` unavailable an amber notice appears and verdicts fall back to permission matching |
| 98 | Utilities — Get Lists — Permissions vs. Licenses | Pick the list and Load; **one row per permission-licence pair** — Permission, Licence, Label, Domain, Entity, Action, Division Aware, Conditions, # Licences — sorted by permission; a permission granted by two licences appears twice with # Licences = 2; a permission in no licence appears once with an empty Licence and 0; the Licence column filter lists individual licences and **only** on one gives exactly that licence's permissions; the row count reads "N rows", not permissions; a horizontal scrollbar sits **above** the table and stays in sync with the one below; Export writes the visible rows; switching lists clears the table and its columns; without `authorization:license:view` the list is **shown disabled** reading “— needs authorization:grant:add or authorization:license:view”, the picker defaults to a list the user can read, and lacking one list does not affect the others |
| 99 | Roles — Permissions vs. Users — STA License mode | Toggle to STA License; the tab names the licence it identified (“Checking against **gcSTAupgrade**”) with Search enabled and nothing to tick; on a **CX 3** org it says the base licence includes STA, disables Search and explains nobody there can hold one; on an org holding **STA but not WEM** (Milestone) Search lists the users behind its STA-inferring roles, split into Unlicensed trigger / Licensed with unique counts in pills — note only roles that **actually have users attached** appear, because only in-use roles are sent to `/license/infer`, so an STA-carrying role nobody holds is correctly absent rather than missing, triggering role, source (manual vs. group) and the responsible permissions — the four STA-only permissions being `billing:user:staUpgrade` and `routing:transcriptionSettings:{view,add,edit}`; on an org holding **both WEM and STA** (3C Retail, while its WEM trial lasts) the tab correctly finds **nobody** and says that `gc2WEMupgrade` took precedence and to check that tab instead — an empty table with no explanation is a failure of this case, and any user listed here at all means precedence stopped being applied; the same user never appears in both the WEM and STA tabs on one org; the **Roles** pill, text filter, cost caveat, `/license/infer` fallback notice and Export to Excel all behave as they do on the WEM tab, with exports named `STA_License_*` |
| 100 | Roles — Create — Templates | Select an org; the **Templates** dropdown lists **Clean Admin** plus one entry per licence the org holds, matching the Licence column of that org's Permissions vs. Licenses export exactly — nothing extra, nothing greyed; picking **Clean Admin** loads ~1,675 permissions on **Demo** (CX 3), ~1,301 on **Milestone** and ~1,413 on **3C Retail** into the ordinary permission builder, and the note names the base licences it covers; the loaded set contains no `billing:user:*` permission and none from Predictive Engagement, Workitems or any other add-on; picking **Clean gcSTAupgrade** on **Milestone** loads all 23, while on **3C Retail** (which also holds WEM) it loads **4 of 23** and names `gc2WEMupgrade` as granting the other 19; picking **agentAssistLicense** on Milestone loads nothing and explains that all 4 of its permissions are also in `agentAssistOmniLicense`, pointing there instead; the Permissions header agrees with the template note — a template reporting 8 permissions shows **8 permissions in 2 entities**, not 2; the role name is pre-filled from the template and editable; **removing a domain in the builder before creating leaves it out of the created role**; selecting a template when the builder already holds permissions asks before replacing them, and **choosing “none, build by hand” asks before clearing them** rather than silently leaving them loaded; declining either prompt **puts the dropdown back to the template that is actually loaded**, never to “none” over a full list;  after Create the page reports the licences Genesys says the role invokes — base licences only for Clean Admin — and if that check cannot run it says so rather than implying the role is clean; **the open question**: on 3C Retail, whether Clean gcSTAupgrade's 4 permissions infer `gcSTAupgrade` (the exclusive rule works) or `gc2WEMupgrade` (an STA-scoped role is impossible there and the wording must change) |
...
### Template schedule not executing at the expected time

- **Cause:** Durable Functions orchestrator may not have been started, or the Timer Function App is stopped
- **Fix:**
  1. Verify `TIMER_FUNCTION_URL` is set on the SWA app settings (pointing to the Timer Function App URL)
  2. Verify `SCHEDULE_RUNNER_KEY` matches between SWA and Timer Function App
  3. Check the template-schedule-starter endpoint: `POST /api/template-schedule-starter` with `{"action":"status","scheduleId":"<id>"}` — should return orchestrator status
  4. Check the Timer Function App logs in Azure Portal for orchestrator errors
  5. Re-save the schedule from the UI to re-trigger the starter notification

### Template schedule starter returns 403

- **Cause:** `SCHEDULE_RUNNER_KEY` mismatch between SWA and Timer Function App
- **Fix:** Ensure the `SCHEDULE_RUNNER_KEY` value is identical in both Azure SWA app settings and the Timer Function App app settings

### Template runner returns 500

- **Cause:** Missing Genesys credentials for the org, or the template/schedule was deleted
- **Fix:** Check SWA function logs; verify the template and schedule still exist; verify `GENESYS_<ID>_CLIENT_ID` and `GENESYS_<ID>_CLIENT_SECRET` are set for the org
...
│   │   │       ├── addUsersToTemplates.js Skill Templates — Add Users To Templates (assign/remove users)
│   │   │       ├── templateSchedules.js Template Schedules — Overview of all template schedules (enable/disable, delete)
...
│   │   │       ├── skillTemplates.js Skill/Role/Queue Templates export + dynamic template picker
...
│       ├── templateAssignmentService.js Template assignment CRUD API wrappers
...
│   ├── templates/
│   │   ├── function.json         HTTP trigger binding (GET/POST/PUT/DELETE)
│   │   └── index.js              CRUD /api/templates → skill template management
│   ├── template-assignments/
│   │   ├── function.json         HTTP trigger binding (GET/POST/DELETE)
│   │   └── index.js              CRUD /api/template-assignments → template-user assignment tracking
│   ├── template-schedules/
│   │   ├── function.json         HTTP trigger binding (GET/POST/PUT/DELETE)
│   │   └── index.js              CRUD /api/template-schedules → template schedule management
│   ├── template-runner/
│   │   ├── function.json         HTTP trigger binding (POST)
│   │   └── index.js              POST /api/template-runner → execute a template schedule
│   └── lib/
...
│       ├── templateAssignmentStore.js Azure Table Storage CRUD for template-user assignments
