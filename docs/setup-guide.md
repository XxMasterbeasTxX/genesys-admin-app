...
- Added **Utilities — Get Lists** page at nav path **Utilities > Get Lists** — a registry-driven viewer for the small reference lists in an org. A **List** dropdown picks which one; the result is shown with per-column filters and click-to-sort headers, and exports to Excel. Read-only. First list: **Presence Definitions** (`GET /api/v2/presence/definitions`, `localeCode=ALL`), flattened to one row per language label. Adding a further list is one entry in `LIST_DEFS` in `js/pages/utilities/getLists.js`. Access key `utilities.getLists`.
