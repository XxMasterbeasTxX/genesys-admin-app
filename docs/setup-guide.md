...
| 97 | Roles — Permissions vs. Users — WEM License mode | Toggle to WEM License; the page names the licence it identified ("Checking against gc2WEMupgrade") and shows only that chip, with **Change** revealing the rest and **Show less** collapsing back; on a `cloudCX3`+ org it says the base licence bundles WEM instead of asking for a selection; an org with neither opens the full list and disables Search until something is ticked; results split into Unlicensed trigger / Licensed / License unused with unique user counts in pills; a user who neither triggers nor holds WEM does not appear; triggering role, source (manual vs. group) and the responsible permissions are shown; permissions granted by the base licence are not listed as triggers; text filter and pills both narrow the table; Export to Excel writes only the visible rows and the full permission list; with `/api/v2/license/infer` unavailable an amber notice appears and verdicts fall back to permission matching |
