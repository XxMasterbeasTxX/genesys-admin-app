...
- **Recordings — Create Export Job** — Create a Genesys Cloud bulk recording export job via `POST /api/v2/recording/jobs`. Filters: date range (required), Integration dropdown (auto-fetched and filtered to storage types `amazon-s3` / `azure-blob-storage` / `google-cloud-storage`), Queues multi-select, Users multi-select, Include Screen Recordings, optional Action Date. Job is created in `READY` state and must be executed from the Export Jobs page. Access key: `interactions.recordings.create`.
- **Recordings — Export Jobs** — List and manage all recording jobs in the selected org (EXPORT/DELETE/ARCHIVE). Expandable detail row shows the original filters (date range, integration name, queues, users, screen recordings, action date, created by) with names resolved and cached. Per-row actions: **Execute** (`READY` jobs only — PUT state `PROCESSING`), **Cancel** (PUT state `CANCELLED`), **Delete** (DELETE `/api/v2/recording/jobs/{id}`). Access key: `interactions.recordings.jobs`.
