...
| POST | `/api/feature-requests` | Create — body: `{ title, description, type?, route?, pageLabel?, orgId?, orgName?, appVersion?, publishAnonymously? }`. Owner and identity come from the token, never the body; every request starts `private`/`new`. Capped at 120/4000 chars and 20 creates per user per 24h. Emails the superusers, and a receipt to whoever filed it. |
| PUT | `/api/feature-requests/{id}` | Submitter edits their own `title`/`description`/`type` **while status is `new`** (409 after triage). Superuser sets `status`, `adminNote`, `shippedVersion`, `duplicateOf`, `visibility`, `sharedTitle`, `sharedDescription`. Promoting to `shared` without a `sharedTitle` is refused. A status change emails the submitter **and** every voter (different wording each); changing only the note or the published text emails nobody. Statuses: `new`, `considering`, `awaiting-submitter`, `planned`, `in-progress`, `shipped`, `not-planned`, `duplicate` — rows written as the older `declined` are mapped to `not-planned` on read, and the older `triaged` to `considering`. |
| POST | `/api/feature-requests/{id}/vote` | Toggle the caller's vote. Permitted on anything the caller can see, so votes on a promoted request aggregate across every org. Idempotent by construction. Records the voter's verified address alongside the vote so status changes can reach them — never projected onto any card. Removing the vote removes the address, and is therefore also the unsubscribe. |
...
| POST | `/api/flow-yaml` | Returns the *structured* Archy YAML of a flow for the **Flow Overview** page — body: `{ orgId, flowName, flowType }`. The SWA function classifies the caller server-side: an internal user may name any org; a customer session gets its own org only (`403 org_locked` otherwise) and must be a named Admin Tool user (`403 user_not_licensed`) holding `architect:flow:view` on their own account (`403 permission_required`); an absent or unverifiable token → `401`. It then forwards to the onboarding runner's `POST /api/export-yaml` using `RUNNER_BASE_URL` + a shared `x-export-key` (`EXPORT_YAML_KEY`). Returns `{ yaml, flowName, flowType }`. The flat REST `latestconfiguration` is deliberately **not** used because it omits implicit *Default* reconvergence links. Runs on the runner because the export needs the Flow Scripting SDK (Node 20+, one SDK session per child process). |
...
Used by: Divisions (all object types), Data Tables — Create, Data Tables — Edit, export pages, All Roles Export, Deployment — Basic, Roles — Compare (all three modes), Roles — Permissions vs. Users, Roles — Create, Roles — Edit, Skill Templates — Add Users To Templates, WebRTC Phones — Create (division filter)
...
permission strings. Flagged **preview** in the JS SDK reference and absent from
the public swagger, but **observed working on 2026-09-09**: the request body is a
bare array of permission strings, mirroring `/license/infer`, and the response is
a bare array of licence ids. Roles — Create → Templates uses it as a pre-flight,
so a permission set can be checked *before* any role exists.

Being preview, it is still treated as optional: every failure is swallowed and
nothing depends on it, because `POST /api/v2/license/infer` runs on the created
role regardless. Do not make it load-bearing — it can be withdrawn without
notice.
